...
#         features = self.l2(features)
        x = x.view(x.size(0), -1)
        x = self.fc1(x) # from 512 dimension to 128
        features = self.relu(x) # use features with non-linearity
        x = self.fc2(features) # final classifier
