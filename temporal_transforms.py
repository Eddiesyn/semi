...
class TemporalRandomCrop(object):
    ...
    def __call__(self, frame_indices):
        ...
        # frame_indices: total length of a clip or sampled length of a clip
        vid_duration = len(frame_indices)
        # size is the desired clip length
        # downsample means each frame is extracted every downsample frame
        # if 1: frame is extracted one by one
        # if 2: frame is extracted every second frame
        clip_duration = self.size * self.downsample
        ...
        # normally frame_indices length is greater than size, if not repeat some frames
        # until a size-length clip is fullfilled
        for index in out:
            if len(out) >= clip_duration:
                break
            out.append(index)
